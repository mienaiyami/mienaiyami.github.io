...
function component(x,y,color,w,h,type){
    ...
    this.crashed = function(obj){
        ...
        if((left <= objRight) && (right >= objRight) && (bottom >= objTop) && (top <= objBottom)){
            crash = [true,'right',objLeft];
        }
        if((right > objLeft) && (left < objRight) && (bottom >= objTop) && (top <= objTop)){
            crash = [true,'top',objTop];
        }
        if((right > objLeft) && (left < objRight) && (top <= objBottom) && (bottom >= objBottom)){
...
function updateGameArea(){
    ...
    myGamePiece1.speedY = 0;
    speedConst = 5
    if (myGameArea.keys && myGameArea.keys[65]) {myGamePiece1.speedX = -speedConst; }
    if (myGameArea.keys && myGameArea.keys[68]) {myGamePiece1.speedX = speedConst; }
    if (myGameArea.keys && myGameArea.keys[87]) {myGamePiece1.speedY = -speedConst; }
    if (myGameArea.keys && myGameArea.keys[83]) {myGamePiece1.speedY = speedConst; }
    myObstacle1.update()
    myGamePiece1.newPos(myGamePiece1.crashed(myObstacle1));
    infoP.append( myGamePiece1.crashed(myObstacle1)[1])
    myGamePiece1.update(myGamePiece1.crashed(myObstacle1).toSrting);
}